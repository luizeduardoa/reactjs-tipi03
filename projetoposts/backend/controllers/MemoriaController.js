    ...
    try {
        ...
        const memory = await Memoria.findById(req.params.id);
            ...
            const removeOldImage = (memory) => {
             ...
             const updateMemory = await Memoria.findByIdAndUpdate(req.params.id, updateData, { nem: true});
             req.json({uptadeMemory, msg: "A memória foi atualizada"})
    } catch(error){
        console.log(error.message);
        res.status(500).send("Error");
...
const alterarFavorito = async (req, res) => {
    try {
        ...
        await memory.save();
        res.json({msg: "Adicionada aos favorito", memory});
...
const adicionarComentario = async (req, res) => {
    try{
        ...
        if(!memory){
            ...
            await memory.save();
            res.json({msg: "O comentário foi adicionada", memory});
    } catch(error){
        console.log(error.message);
        res.status(500).send("Error");
    }
